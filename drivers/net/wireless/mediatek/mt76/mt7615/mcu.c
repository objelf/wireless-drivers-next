...
static void
mt7615_mcu_debug_msg_event(struct mt7615_dev *dev, struct sk_buff *skb)
{
	struct mt7615_mcu_rxd *rxd = (struct mt7615_mcu_rxd *)skb->data;
	struct debug_msg {
...
mt7615_mcu_rx_unsolicited_event(struct mt7615_dev *dev, struct sk_buff *skb)
{
	...
	case MCU_EVENT_DBG_MSG:
		mt7615_mcu_debug_msg_event(dev, skb);
		break;
	case MCU_EVENT_CSA_DONE:
		ieee80211_iterate_active_interfaces_atomic(dev->mt76.hw,
				IEEE80211_IFACE_ITER_RESUME_ALL,
				mt7615_mcu_csa_finish, dev);
		break;
...
static int
mt7615_mcu_set_wmm(struct mt7615_dev *dev, u8 queue,
		   struct mt7615_vif_wmm *wmm)
{
	struct {
		u8 number;
		u8 rsv[3];
		...
		.txop = cpu_to_le16(wmm->txop),
	};

	if (wmm->cw_min)
		req.cw_min = fls(wmm->cw_min);
	if (wmm->cw_max)
...
int mt7615_mcu_set_tx(struct mt7615_dev *dev, struct ieee80211_vif *vif)
{
	...
	for (i = 0; i < IEEE80211_NUM_ACS; i++) {
		...
		queue = mt7615_lmac_mapping(dev, i);
...
static int
mt7615_mcu_set_arp_filter(struct mt7615_dev *dev,
			  struct ieee80211_vif *vif,
			  bool suspend)
{
	struct mt7615_vif *mvif = (struct mt7615_vif *)vif->drv_priv;
