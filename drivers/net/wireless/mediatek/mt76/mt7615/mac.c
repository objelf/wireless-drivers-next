			  ...
			  struct ieee80211_key_conf *key, bool beacon)
{
	...
	bool is_sf = mt76_is_usb(&dev->mt76) || mt76_is_sdio(&dev->mt76);
	int tx_count = 8;
	u8 fc_type, fc_stype, p_fmt, q_idx, omac_idx = 0, wmm_idx = 0;
	__le16 fc = hdr->frame_control;
	u32 val, sz_txd = is_sf ? MT_USB_TXD_SIZE : MT_TXD_SIZE;
	u16 seqno = 0;
	...
	if (beacon) {
		...
		p_fmt = is_sf ? MT_TX_TYPE_SF : MT_TX_TYPE_CT;
			  ...
			  struct ieee80211_tx_rate *rates)
{
	...
	if (mt76_is_usb(&dev->mt76) || mt76_is_sdio(&dev->mt76)) {
